...
class SerializationHelper {
  ...
  static Map<String, dynamic> serializeRow(
    ...
    for (final entry in row.entries) {
      ...
      if (value is Uint8List || (value is List && value.isNotEmpty && value.first is int)) {
  ...
  static Map<String, dynamic> deserializeRow(
    ...
    for (final entry in row.entries) {
      ...
      if (binaryFields != null && binaryFields.contains(key) && value is String) {
        try {
  ...
  static List<Map<String, dynamic>> serializeRows(
    ...
    return rows.map((row) => serializeRow(row, binaryFields: binaryFields)).toList();
  }
  ...
  static List<Map<String, dynamic>> deserializeRows(
    ...
    return rows.map((row) => deserializeRow(row, binaryFields: binaryFields)).toList();
  }
}
